...
class ChannelRelayAPITest(TestCase):
    ...
    def test_channel_create(self):
        ...
        self.assertEqual(data['channel'], ChannelRelay.objects.get(master = SignClient.objects.get(externid = self.clientId), name = 'testchannel', status = ChannelRelay.STATUS_ALIVE).pk)

    def test_channel_delete(self):
        c = Client()
        response = c.post("/api/1.0/channeladd?s="+self.clientId, {'name': 'testchannel'})
        channelid = json.loads(response.content)['channel']
        self.assertEqual(len(ChannelRelay.objects.filter(pk = channelid)), 1)
        ...
        with self.assertRaises(ChannelRelay.DoesNotExist):
            ChannelRelay.objects.get(pk = channelid, status=ChannelRelay.STATUS_ALIVE)
