    ...
    $scope._p2pConnectionStateChange = function (r, state) {
        ...
        if ( $scope.remotes._indexOfS(r) >= 0 ) {
            ...
            if (state === "connected") {
                ...
                $scope.hide_broadcast_status = true;
                $scope.$digest();
...
//            );
    ...
    $scope.startStreaming = function(channel_id) {
    ...
    */
    ...
    $scope.setFullScreen = function (video) {
    }
...
$(document).ready( function () {
    ...
    var customMessageBus = window.castReceiverManager.getCastMessageBus('urn:x-cast:io.rtccast.demo');
        customMessageBus.onMessage = function(event) {
            d = JSON.parse(event.data);
            console.log("got message", d);
            scope.startStreaming(d.channel_id);
    }
